...
| `background` | solid color form, `linear-gradient()` (2+ stops → first+last, `90/270deg`/`to left/right` = horizontal, `0/180deg`/`to top/bottom` = vertical), `radial-gradient(circle at X% Y%, A p%, B q%)`, `conic-gradient(from Ndeg, C0 P0[, C1 P1...])` (2–4 stops, positions in `deg`/bare 0–360, first 0 last 360 after clamping; center fixed 50%/50%) | comma layers split paren-aware; scanned base-first (last layer first), first supported form wins — texture overlays (`repeating-*`, `url(`) are skipped with a warning, so `.amp`'s brushed layer falls through to its linear base; unknown angles/px offsets/ellipse/`at` center/>4 stops/`%` conic positions skip the layer the same way |
